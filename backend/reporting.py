...
def generate_charter_decision_pdf(
    ...
    data_mode: str = "DEMO",
    synthetic_validation: Optional[Dict[str, Any]] = None,
    real_validation: Optional[Any] = None,
) -> bytes:
    """
    ...
    # SECTION 7: FORECAST VALIDATION STATUS
    story.append(Paragraph("7. FORECAST VALIDATION STATUS", style_heading))

    # Synthetic validation row
    if synthetic_validation and synthetic_validation.get("status") not in (None, "UNAVAILABLE"):
        sv_mae = synthetic_validation.get("MAE")
        sv_rmse = synthetic_validation.get("RMSE")
        sv_mape = synthetic_validation.get("MAPE")
        sv_status = synthetic_validation.get("status", "?")
        if sv_status == "OK" and sv_mae is not None:
            story.append(Paragraph(
                f"Synthetic demo-series validation (SYNTHETIC data): "
                f"MAE = {format_pdf_inr_val(usd_to_inr(sv_mae))} / t, "
                f"RMSE = {format_pdf_inr_val(usd_to_inr(sv_rmse))} / t, "
                f"MAPE = {sv_mape:.2f}%. "
                "These metrics are computed on the same synthetic series used for model fitting — "
                "they do not establish real-market forecast accuracy.",
                style_body
            ))
        else:
            story.append(Paragraph(
                f"Synthetic demo-series validation: status = {sv_status}. "
                "Insufficient data for error estimation on this run.",
                style_body
            ))
    else:
        story.append(Paragraph(
    ...
    if real_validation is not None:
        rv_status = getattr(real_validation, "status", "UNAVAILABLE")
        rv_mae = getattr(real_validation, "mae", None)
        rv_rmse = getattr(real_validation, "rmse", None)
        rv_mape = getattr(real_validation, "mape", None)
        rv_n = getattr(real_validation, "n_observations", None)
        rv_source = getattr(real_validation, "source_name", "Open-Meteo")
        rv_notes = getattr(real_validation, "notes", "")

        if rv_status == "UNAVAILABLE":
            story.append(Paragraph(
                f"External validation ({rv_source}): UNAVAILABLE — no cached series and network unreachable.",
                style_body
            ))
        elif rv_status == "INSUFFICIENT_DATA":
            story.append(Paragraph(
                f"External validation ({rv_source}): INSUFFICIENT DATA — {rv_notes}",
                style_body
            ))
        else:
            story.append(Paragraph(
                f"External validation ({rv_source}, PUBLIC_REAL data): "
                f"MAE = {rv_mae:.3f} m/s, RMSE = {rv_rmse:.3f} m/s, MAPE = {rv_mape:.2f}%. "
                f"Observations: {rv_n}. {rv_notes}",
                style_body
            ))
    else:
        story.append(Paragraph(
    ...
    # SECTION 8: DATA PROVENANCE & DISCLOSURES
    story.append(Paragraph("8. DATA PROVENANCE & OPERATIONAL DISCLOSURES", style_heading))
    story.append(Paragraph(f"• Data mode for this report: {data_mode}.", style_bullet))
